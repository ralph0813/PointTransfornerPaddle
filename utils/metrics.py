"""
cited from https://github.com/guochengqian/openpoints/blob/master/utils/metrics.py
"""
...
class ConfusionMatrix:
    ...
    def update(self, pred, true):
        ...
        bins = paddle.bincount(unique_mapping, minlength=self.virtual_num_classes ** 2)
        self.value += bins.reshape((self.virtual_num_classes, self.virtual_num_classes))[:self.num_classes,
                      :self.num_classes]
    ...
    @property
    def union(self):
        return self.value.sum(axis=0) + self.value.sum(axis=1) - paddle.diag(self.value)
    ...
    def cal_acc(tp, count):
        ...
        macc = paddle.mean(acc_per_cls)  # class accuracy
    ...
    def print_acc(accs):
        ...
        out += '\n' + '-' * 20
        out += '\n' + '   Mean  ' + f'{paddle.mean(accs).item():.2f}'.rjust(8)
        logging.info(out)
